...
class BertText49Pooler(nn.Module):
    def __init__(self, config):
        super(BertText49Pooler, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.activation = nn.Tanh()
    ...
    def forward(self, hidden_states):
        # We "pool" the model by simply taking the hidden state corresponding
        # to the first token of text.
        first_token_tensor = hidden_states[:, 49]
        pooled_output = self.dense(first_token_tensor)
        pooled_output = self.activation(pooled_output)
        return pooled_output

class BertText16Pooler(nn.Module):
    def __init__(self, config):
        super(BertText16Pooler, self).__init__()
        self.config = config
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.activation = nn.Tanh()
...
class ResBertForMMSequenceClassification(PreTrainedBertModel):
    """Entity-Attention BERT model for classification with text and image inputs, 1+text. Res-BERT+BL w TFN
    """
    def __init__(self, config, num_labels=2, bert_layer=True, tfn=True):
        super(ResBertForMMSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
    ...
    def forward(self, input_ids, s2_input_ids, visual_embeds_att, token_type_ids=None, s2_type_ids=None,
        ...
        if self.bert_layer:
            ...
            final_text_output = pooled_output
        ...
        if self.tfn:
            dot_matrix = torch.bmm(final_text_output.unsqueeze(2), converted_vis_embed_map.unsqueeze(1))
            dot_map = dot_matrix.view(-1, self.config.hidden_size*self.config.hidden_size)
            pooled_output = self.dropout(dot_map)
        else:
            final_output = torch.cat((final_text_output, converted_vis_embed_map), dim=-1)
...
class MBertNoPoolingForMMSequenceClassification(PreTrainedBertModel):
    """BERT model for classification with text and image inputs, 49+text (MBERT I without pooling) havenot run
    """
    def __init__(self, config, num_labels=2):
        super(MBertNoPoolingForMMSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.vismap2text = nn.Linear(2048, config.hidden_size)
        #self.img_attention = BertLayer(config)
        self.comb_attention = MultimodalEncoder(config)
        self.text_pooler = BertText49Pooler(config)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, s2_input_ids, visual_embeds_att, token_type_ids=None, s2_type_ids=None,
        ...
        text_img_output = torch.cat((converted_vis_embed_map, sequence_output), dim=1)

        extended_attention_mask = added_attention_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        ...
        multimodal_encoder = self.comb_attention(text_img_output, extended_attention_mask)
        img_att_text_output_layer = multimodal_encoder[-1]
        img_att_text_output = self.text_pooler(img_att_text_output_layer)

        pooled_output = self.dropout(img_att_text_output)
        logits = self.classifier(pooled_output)
