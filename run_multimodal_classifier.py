...
Acknowledgements: 
Coding based on https://github.com/huggingface/transformers and https://github.com/jefferyYu
"""
...
from my_bert.mm_modeling import (ResBertForMMSequenceClassification, MBertForMMSequenceClassification,
                                 MBertNoPoolingForMMSequenceClassification)
...
def convert_mm_examples_to_features(examples, label_list, max_seq_length, max_entity_length, tokenizer, crop_size,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        try:
            ...
            image_path_fail = os.path.join(path_img, '813GgT2-9BL._SY88.jpg')
...
def main():
    ...
    parser.add_argument('--mm_model', default='MBert',
                        help='model name')  # MBert, MBertNoPooling, ResBert
    ...
    if args.task_name == "amazon":  # this refers to amazon dataset
        args.path_image = "./data/amazon_images/"
    else:
        print("The task name is not right!")

    processors = {
        "amazon": AbmsaProcessor,  # amazon dataset
    }

    num_labels_task = {
        "amazon": 2,  # amazon dataset
    }
